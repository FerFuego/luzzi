...
if ( ! function_exists( 'shopper_cart_link' ) ) {
	...
	function shopper_cart_link( $isCart = true ) {
		?>
			<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="Ver Carrito">
				<?php if ($isCart) : ?>
				<span class="label-cart"><?php _e('Carito', 'shopper') ?> / </span> 
				<?php endif ?>
		...
		if ( shopper_is_woocommerce_activated() ) { 
			...
			<div class="custom-product-search">
				<form role="search" method="get" class="shopper-product-search" action="<?php echo esc_url( home_url( '/' ) ); ?>">
					<div class="nav-left">
						<div class="nav-search-facade" data-value="search-alias=aps"><span class="nav-search-label"><?php _e( 'Todos', 'shopper' ); ?></span> <i class="fa fa-angle-down"></i></div>			
						<?php
					...
					<div class="nav-fill">
						...
						<input name="s" type="text" value="<?php echo get_search_query(); ?>" placeholder="<?php esc_attr_e( 'Buscar productos', 'shopper' ); ?>"/>
					</div>
				</form>
			</div>
			<?php
...
if ( !function_exists( 'shopper_header_myacount' ) ) {
	...
	function shopper_header_myacount() {
		...
		?>
			...
			<a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('Mi Cuenta','shopper'); ?>"><i class="fa fa-user"></i></a>
			<?php

		} else {

			?>
			<a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('Ingresar / Registrarse','shopper'); ?>"><i class="fa fa-lock"></i></a>
			<?php 
		}
...
if ( ! function_exists( 'shopper_handheld_footer_bar_search' ) ) {
	...
	function shopper_handheld_footer_bar_search() {
		echo '<a href="">' . esc_attr__( 'Buscar', 'shopper' ) . '</a>';
...
if ( ! function_exists( 'shopper_handheld_footer_bar_cart_link' ) ) {
	...
	function shopper_handheld_footer_bar_cart_link() {
		?>
			<a class="footer-cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="Ver carrito">
				<span class="count"><?php echo wp_kses_data( WC()->cart->get_cart_contents_count() );?></span>
			</a>
		<?php
	}
...
if ( ! function_exists( 'shopper_handheld_footer_bar_account_link' ) ) {
	...
	function shopper_handheld_footer_bar_account_link() {
		echo '<a href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_attr__( 'Mi Cuenta', 'shopper' ) . '</a>';
	}
}
...
if ( ! function_exists( 'shopper_template_loop_category_title' ) ) {
	...
	function shopper_template_loop_category_title( $category ) {
		...
		$count_html = ' <span class="count">' . sprintf( _n( '%s Producto', '%s Productos', $category->count, 'shopper' ), $category->count )  . '</span>';
